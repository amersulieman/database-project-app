...
Find the cheapest course to make up one’s skill gap by showing the course to take and the cost (of the section price). 5 */
...
right_course as( SELECT c_code,title
                ...
                HAVING COUNT(ks_code) = (select count(ks_code)
                                        from missing))
/*return the course with the cheapest section price*/                                    
